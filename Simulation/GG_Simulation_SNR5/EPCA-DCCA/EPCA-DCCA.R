...
for (i in 1:nrep){
  ...
  res_DCCA_mu1_list = append(res_DCCA_mu1_list, list(colMeans(X1)))
  res_DCCA_mu2_list = append(res_DCCA_mu2_list, list(colMeans(X2)))
  
  theta1_epca = (X1 - tcrossprod(ones, para1$mu)) %*% tcrossprod(para1$U)
  theta2_epca = (X2 - tcrossprod(ones, para2$mu)) %*% tcrossprod(para2$U)
  
  theta1 = X1 - tcrossprod(ones, colMeans(X1))
  theta2 = X2 - tcrossprod(ones, colMeans(X2))
...
save(res_DCCA_list, res_EPCA_DCCA_list, res_DCCA_list_rank1, res_EPCA_DCCA_list_rank1, 
     res_DCCA_mu1_list, res_DCCA_mu2_list, res_EPCA_DCCA_mu1_list, res_EPCA_DCCA_mu2_list, file = "Simulation/GG_Simulation_SNR5/DCCA_result.RData")
