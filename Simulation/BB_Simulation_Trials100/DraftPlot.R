...
###################### ECCA ##########################
for (i in 1:nrep){
  ECCA_signal_error1[i] = Fnorm(output[[i]]$theta1_est - theta1_true[[i]])^2/Fnorm(theta1_true[[i]])^2
  ECCA_signal_error2[i] = Fnorm(output[[i]]$theta2_est - theta2_true[[i]])^2/Fnorm(theta2_true[[i]])^2
...
fig.path = "Simulation/BB_Simulation_Trials100/Figures/"
pdf(file = paste(fig.path,"Whole_Estimation_Error_BB_SNR5_Draft.pdf",sep=""), width = 16, height = 9)
print(gg)
dev.off()

fig.path = "Simulation/BB_Simulation_Trials100/Figures/"
pdf(file = paste(fig.path,"Whole_Joint_Error_BB_SNR5_subspace_Draft.pdf",sep=""), width = 16, height = 9)
print(gg_joint)
dev.off()